...
Der neue `Publisher` publiziert nur die explizit mit `outFormats` angeforderten Formate. Die Angabe ist zwingend;
die Transferdatei wird nicht automatisch beigepackt. Unterstützte Werte sind `xtf`, `itf`, `gpkg`, `shp`, `dxf`
und `dxf_geobau`. `xtf` und `itf` müssen zur Erweiterung der Quelle passen.

```groovy
tasks.register('publishShapefile', Publisher) {
    // ... source and output configuration ...
    outFormats = ["shp"]
}

tasks.register('publishTransferAndGpkg', Publisher) {
    // ... source and output configuration ...
    outFormats = ["xtf", "gpkg"]
}
```
...
Standardmässig schreibt `Publisher` Metadaten in die Publikationsdatenbank und erzeugt `meta/metainfo.json`.
Die globalen Publisher-Einstellungen, einschliesslich `pupFolderPath`, bleiben dafür erforderlich. Mit
`outFolderPath` kann der Zielordner für einen einzelnen Task überschrieben werden.

Für eine rein lokale Ausgabe ohne Metadaten sind keine globalen Publisher-Einstellungen nötig:

```groovy
tasks.register('publishLocal', Publisher) {
...
In diesem Modus werden die aufgelösten `.ili`-Modelle nach `meta/` kopiert, aber weder `meta/metainfo.json` noch
Publikationsdatenbank-Zeilen geschrieben.
