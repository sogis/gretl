...
public class ShpExportTest {
    ...
    public ShpExportTest() {
        this.log = LogEnvironment.getLogger(SqlReader.class);
    }
    ...
    public void exportOk() throws Exception {
        ...
        try (
            ...
            stmt.execute("CREATE TABLE " + schemaName + ".exportdata(t_id serial, \"Aint\" integer, adec decimal(7,1), atext varchar(40), aenum varchar(120),adate date, atimstamp timestamp, aboolean boolean,geom_so geometry(POINT,2056))");
            stmt.execute("INSERT INTO " + schemaName + ".exportdata(t_id, \"Aint\", adec, atext, adate, atimstamp, aboolean,geom_so) VALUES (1,2,3.4,'abc','2013-10-21','2015-02-16T08:35:45.000','true',ST_GeomFromText('POINT(2638000.0 1175250.0)',2056))");
            stmt.execute("INSERT INTO " + schemaName + ".exportdata(t_id) VALUES (2)");
        ...
        try (
            ...
            try (SimpleFeatureIterator featureCollectionIter = featuresSource.getFeatures().features()) {
                SimpleFeature shapeObj = featureCollectionIter.next();
